    ...
    <div>
        ...
        {tags.map((tag) => (
          <button
            ...
            className={`px-4 py-2 rounded-full transition-all ${
              activeTag === tag.id ? "bg-accent-blue text-white" : "bg-light text-dark hover:bg-accent-orange/10"
            }`}
          >
            {tag.label}
          </button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {filteredImages.map((image) => (
          <div key={image.id} className="relative aspect-[3/4] rounded-2xl overflow-hidden group">
            <Image
